...
void Controlador_editor::cargar_mapa()
{
	...
	importador.importar(nombre_fichero.c_str(), mapa.obstaculos, mapa.decoraciones);
	obtener_desde_mapa();
	mapa.limpiar();
	reordenar_decoraciones();
...
void Controlador_editor::grabar_mapa()
{
	...
	aplicar_a_mapa(mapa);
	fichero<<exportador.serializar(mapa.obstaculos, mapa.decoraciones);
...
void Controlador_editor::intercambiar_objeto_creado()
{
	switch(tobjeto)
	{
		...
		case tobjetocreado::decoracion: 	tobjeto=tobjetocreado::obstaculo; break;
	}
}
